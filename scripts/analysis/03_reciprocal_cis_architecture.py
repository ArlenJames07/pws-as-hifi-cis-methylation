...
def build_report(
    ...
    add("smoothing", "gap_safe_rolling_median", f"centred median of {ROLLING_WINDOWS} contiguous evaluable 1-kb windows (minimum {ROLLING_MIN_WINDOWS}); never crosses a non-evaluable window; {gap_detail}", "pass")
    diploid = table[table["mechanism"].isin(DIPLOID_GROUPS)]
    add(
        "phase_invariant_asm",
